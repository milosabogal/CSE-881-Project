...
WINDOW_SIZE_VALUES = [3]  # [3, 5, 10, 20]
BATCH_SIZE_VALUES = [16]  # [16, 32, 64, 128]
HIDDEN_SIZE_VALUES = [64]  # [32, 64, 128]
LEARNING_RATE_VALUES = [0.0001]  # [0.1, 0.01, 0.001, 0.0001]
EPOCH_VALUES = [300]  # [50, 100, 300, 1000]
...
class PortfolioOptimizer:
    ...
    def __init__(self, ticker_symbols, start_date=START_DATE, end_date=END_DATE, train_size=TRAIN_SIZE):
    ...
    def construct_features(self, target_feature="Returns"):
        ...
        total_num_days = len(self.dataframes[self.ticker_symbols[0]])
    ...
    def cross_validate(self, full_data, num_folds=NUM_FOLDS, ticker_symbol=None):
        best_error = float("inf")
        best_params = None

        for (window_size, batch_size, hidden_dim, learning_rate, num_epochs) in product(WINDOW_SIZE_VALUES,
                                                                                        BATCH_SIZE_VALUES,
                                                                                        HIDDEN_SIZE_VALUES,
            ...
            for train_indices, test_indices in btscv.split(full_data, window_size):
                ...
                total_error += \
                    self.train_and_test(train, test, window_size, batch_size, hidden_dim, learning_rate, num_epochs,
                                        ticker_symbol)[0]
            average_error = total_error / num_folds
            if average_error < best_error:
                best_error = average_error
                best_params = (window_size, batch_size, hidden_dim, learning_rate, num_epochs)
    ...
    def train_and_test(self, train, test, window_size, batch_size, hidden_dim, learning_rate, num_epochs,
        ...
        if self.method == "regression":
            ...
            one_hot_train_labels = np.eye(3)[
                train_labels.astype(int).reshape(-1)]  # Add one-hot encoding of labels from previous days as features
            one_hot_test_labels = np.eye(3)[
                test_labels.astype(int).reshape(-1)]  # Add one-hot encoding of labels from previous days as features
        ...
        for epoch in range(num_epochs + 1):
            ...
            with torch.no_grad():
                ...
                if self.method == "regression":
                    ...
                    y_pred_orig = scaler.inverse_transform(np.repeat(y_pred, X_train.shape[2], axis=-1))[:, -1]
                    y_train_orig = scaler.inverse_transform(np.repeat(y_train, X_train.shape[2], axis=-1))[:, -1]
        ...
        with torch.no_grad():
            ...
            if self.method == "regression":
                ...
                if final_predictions:
                    pred_probabilities = torch.softmax(y_pred, dim=1).numpy()
    ...
    def train_regression(self):
        for ticker_symbol in self.ticker_symbols:
            ...
            window_size, batch_size, hidden_dim, learning_rate, num_epochs = self.cross_validate(X_validation,
                                                                                                 ticker_symbol=ticker_symbol)
            print("\nBest parameters found:", window_size, batch_size, hidden_dim, learning_rate, num_epochs, "\n")
            train, test = X[:train_size], X[train_size - window_size:]
            self.best_regression_models[ticker_symbol] = \
                self.train_and_test(train, test, window_size, batch_size, hidden_dim, learning_rate, num_epochs,
                                    ticker_symbol, print_progress=False, display_metrics=True, final_predictions=True)[
                    1]
    ...
    def train_classification(self):
        ...
        self.best_classification_model = \
            self.train_and_test(train, test, window_size, batch_size, hidden_dim, learning_rate, num_epochs,
                                print_progress=False, display_metrics=True, final_predictions=True)[1]
    ...
    def get_weights(self, day=-1):
        if self.method == "equal":
            ...
            if self.greedy_classification:
                weights = np.eye(3)[np.argmax(weights)]
            return weights
    ...
    def _plot_results(self, portfolio_returns_list):
        ...
        secax = ax2.secondary_yaxis('right', functions=(lambda x: self.DEFAULT_DAILY_INVESTMENT_AMOUNT * (x / 100 + 1),
                                                        lambda x: 100 * (x / self.DEFAULT_DAILY_INVESTMENT_AMOUNT - 1)))
        secax.set_ylabel('Capital (in USD)')
        plt.xlabel('Day')
