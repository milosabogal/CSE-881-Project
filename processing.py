...
def markowitz_mean_variance(returns_list, covariance_matrix, risk_tolerance=0):
    ...
    Args:
        returns_list: Array/list of the tickers' returns
    ...
    returns_float = returns_list.astype(float)
    P = covariance_matrix
    q = -risk_tolerance * returns_float
    G = -np.eye(len(returns_float))
    h = np.zeros((len(returns_float), 1))
    A = np.ones((1, len(returns_float)))
    b = 1.0
